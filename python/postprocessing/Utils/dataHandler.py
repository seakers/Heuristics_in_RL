...
class DataHandler:
    ...
    # method to obtain the heuristic weights as arrays for each NFE (only for the coevolutionary result files)
    def get_heur_weights(self):
        heur_weights = [-1, -1]
        if not self.read_complete:
            print('Error: File not read')
        else:
            heur_weight_cols = [self.columns[col] for col in self.columns.keys() if 'Heuristic' in col]
            heur_weights = np.zeros((len(heur_weight_cols[0]),len(heur_weight_cols)))
            for i in range(len(heur_weight_cols[0])): # number of heuristic weight combinations
                heur_weight = np.zeros((len(heur_weight_cols)))
                for j in range(len(heur_weight_cols)): # number of heuristics
                    heur_weight[j] = heur_weight_cols[j][i]
                heur_weights[i] = heur_weight
        return heur_weights
