...
class MOORunStatistics:
    ...
    def __init__(self, *args):
        self.objectives = args[0]
        if len(args) > 1:
            self.aggr_constraints = args[1] # aggr_constraints -> sum of absolute constraint violations
        else:
    ...
    def compute_hv(self, *args):
        ...
        if len(population) > 0:
            ...
            self.hypervolume = hv_object.compute([1.1,1.1])/1.1**2
        else:
