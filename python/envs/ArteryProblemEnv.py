...
class ArteryProblemEnv(gym.Env):
    def __init__(self, operations_instance, n_actions, n_states, max_steps, model_sel, sel, sidenum, rad, E_mod, c_target, nuc_fac, save_path, obj_names, constr_names, heur_names, heurs_used, render_steps):
        ...
        self.metamat_support = MetamaterialSupport(sel=sel, operations_instance=operations_instance, sidenum=sidenum, rad=rad, E_mod=E_mod, c_target=c_target, nuc_fac=nuc_fac, n_vars=n_states, model_sel=model_sel, artery_prob=True, save_path=save_path, obj_names=obj_names, constr_names=constr_names, heur_names=heur_names, heurs_used=heurs_used)
