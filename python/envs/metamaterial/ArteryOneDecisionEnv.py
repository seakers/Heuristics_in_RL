...
class ArteryOneDecisionEnv(gym.Env):
    def __init__(self, operations_instance, use_python_model, n_states, model_sel, sel, sidenum, rad, E_mod, c_target, c_target_delta, nuc_fac, save_path, obj_names, constr_names, heur_names, heurs_used, render_steps, new_reward, obj_max, include_wts_in_state):

        super(ArteryOneDecisionEnv, self).__init__()
        ...
        self.new_reward = new_reward # Boolean representing the use of compute_reward() (current PF dominance and diversity-based) or compute_reward2() (current (and previous) state(s) dominance based)         
        ...
        if self.include_weights_in_state:
            self.observation_space = spaces.Dict(
                {
                    "design": spaces.MultiBinary(nvec),
            ...
            self.observation_space = spaces.MultiBinary(nvec)

        self.metamat_support = MetamaterialSupport(operations_instance=operations_instance, use_python_model=use_python_model, sel=sel, sidenum=sidenum, rad=rad, E_mod=E_mod, c_target=c_target, c_target_delta=c_target_delta, nuc_fac=nuc_fac, n_vars=n_states, model_sel=model_sel, artery_prob=False, save_path=save_path, obj_names=obj_names, constr_names=constr_names, heur_names=heur_names, heurs_used=heurs_used, new_reward=new_reward, obj_max=obj_max, obs_space=self.observation_space, include_weights=include_wts_in_state)
    ...
    def step(self, action):
        ...
        # Get action members
        self.action_members, member_added = self.metamat_support.obtain_action_members(current_design=self.current_pos, new_design=new_pos)
