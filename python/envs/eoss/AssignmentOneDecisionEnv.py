...
class AssignmentOneDecisionEnv(gym.Env):
    def __init__(self, operations_instance, resources_path, obj_names, heur_names, heurs_used, consider_feas, dc_thresh, mass_thresh, pe_thresh, ic_thresh, render_steps, new_reward, obj_max, include_wts_in_state):
        super(AssignmentOneDecisionEnv, self).__init__()
    ...
    def step(self, action):
        ...
        assign_idx, new_pos = self.eoss_support.assign_dec_by_action(self.current_pos, action, self.observation_space)

        # Get instrument modified (added/removed) by action
        instr_modified, orbit_modified = self.eoss_support.obtain_action_modified_instrument(actions=action, one_dec=True, assign_idx=assign_idx)

        # Compute Reward Function
        reward, mod_nfe, new_eoss_des = self.eoss_support.compute_reward_one_dec(state=new_pos, nfe_val=self.current_nfe_val, assign_prob=True)
