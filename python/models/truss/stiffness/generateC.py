    ...
    try:
        ...
        for y in range(3):
            ...
            if e11 != 0 or e22 != 0:  # normal x strain, normal y strain
                ...
                # if not np.linalg.matrix_rank(K_qq) == K_qq.shape[0]:
                #     print('K_qq is rank deficient')
            ...
            if np.any(np.isnan(C)):
                # Define a bad stiffness tensor in case stiffness tensor has nans
                C = np.array([[1e-6, 1e-3, 1e3], [1e-3, 1e-3, 1e3], [1e3, 1e3, 1e-3]])
                # mainly C11 = 1e-6 and C22 = 1e-3 (for the equal stiffness problem), C12 = C21 = 1e-3, C61 = C62 = C16 = C26 = 1e3, C66 = 1e-3 (for the artery problem)

    except:
        # Define a bad stiffness tensor in case K_qq is rank deficient (i.e. in case displacements at free nodes cannot be solved) similar to the java problem classes
        C = np.array([[1e-6, 1e-3, 1e3], [1e-3, 1e-3, 1e3], [1e3, 1e3, 1e-3]])
        # mainly C11 = 1e-6 and C22 = 1e-3 (for the equal stiffness problem), C12 = C21 = 1e-3, C61 = C62 = C16 = C26 = 1e3, C66 = 1e-3 (for the artery problem)
