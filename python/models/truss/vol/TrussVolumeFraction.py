...
# from models.truss.vol.c_geometry import voxelize_space, generateNC ### removed c_geometry from the original repository 
from models.truss.stiffness import generateNC
...
class TrussVolumeFraction:
    ...
    def __init__(self, sidenum, bit_list, side_length=100e-5):
        ...
        self.config_obj = Config(sidenum=sidenum)
        bit_connection_list = []

        for x in range(self.config_obj.num_vars):
            design = [0 for _ in range(self.config_obj.num_vars)]
            design[x] = 1
            truss_features = TrussFeatures(design, sidenum, None)
        ...
        self.bit_connection_list = bit_connection_list

        self.n = self.config_obj.sidenum_nvar_map[self.sidenum]
    ...
    def get_bit_connections(self, bit_idx):
        return self.bit_connection_list[bit_idx]


    def get_bits_from_connection(self, connection):
        bits = []
        for idx, bit_conns in enumerate(self.bit_connection_list):
    ...
    #     return volume_fraction, 1.0, [0 for _ in range(self.config_obj.num_vars)]
    ...
    def evaluate2(self, member_radii=50e-6, side_length=100e-5):
        ...
        interaction_vector = [0 for _ in range(self.config_obj.num_vars)]
    ...
    def get_interaction_vec(self, bit_interactions):
        bit_list = [0 for _ in range(self.config_obj.num_vars)]
        for bit, interactions in bit_interactions.items():
            bit_list[bit] = 1
